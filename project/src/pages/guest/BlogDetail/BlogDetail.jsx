...
function BlogDetail({ params }) {
  ...
  return (
    <AnimatedPage ariaLabel={`${post.title} article page`}>
      <article className='bg-white pb-20 pt-32 sm:pb-24 sm:pt-40'>
        <header className='mx-auto w-full max-w-[1600px] px-4 sm:px-6 lg:px-[clamp(2.5rem,4vw,4.75rem)]'>
          <p className='text-xs font-semibold uppercase tracking-[0.16em] text-black/40'>{post.category}{post.draft ? ' · Draft preview' : ''}</p>
        ...
        <div className='mx-auto grid w-full max-w-[1600px] gap-12 px-4 pt-14 sm:px-6 lg:grid-cols-[minmax(0,1fr)_14rem] lg:px-[clamp(2.5rem,4vw,4.75rem)]'>
          ...
          <aside className='lg:sticky lg:top-32 lg:self-start'><p className='mb-4 text-xs font-semibold uppercase tracking-[0.14em] text-black/40'>Topics</p><BlogTagList tags={post.tags} /></aside>
        </div>
