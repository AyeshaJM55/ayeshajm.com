...
function FeaturedWork() {
  const { content, locale, localizePath, t } = useLocale()
  const copy = content.pages.home.featuredWork
    ...
    <section aria-labelledby='featured-work-title' className='bg-white py-16 sm:py-20 lg:py-24' id='portfolio'>
      <div className='mx-auto w-full max-w-[1600px] px-4 sm:px-6 lg:px-[clamp(2.5rem,4vw,4.75rem)]'>
        <header className='mb-8 sm:mb-10'>
          <h2 className='text-3xl font-semibold tracking-[-0.035em] text-black sm:text-4xl lg:text-5xl' id='featured-work-title'>{copy.title}</h2>
          <p className='mt-3 max-w-2xl text-base leading-7 text-black/55 sm:text-lg'>{copy.description}</p>
        </header>
        <div className='grid grid-cols-1 gap-4 py-4 sm:grid-cols-2 sm:py-6 lg:grid-cols-4'>
          {projects.map((project) => (
            <article key={project.slug}>
              <a aria-label={t('accessibility.openProject', { title: project.title })} className='group relative block aspect-square overflow-hidden bg-neutral-100 outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-4' href={localizePath(`/work/${project.slug}`)}>
                <img alt={project.coverAlt} className='h-full w-full object-contain object-top transition-transform duration-700 ease-out group-hover:scale-[1.06] group-focus-visible:scale-[1.06]' height='900' loading='lazy' src={project.coverImage} width='900' />
                ...
                <div className='absolute inset-x-0 bottom-0 z-10 p-5 sm:p-6'>
                  <h3 className='text-xl font-semibold tracking-[-0.025em] text-white sm:text-2xl'>{project.title}</h3>
