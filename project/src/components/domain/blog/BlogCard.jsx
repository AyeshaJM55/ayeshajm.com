...
function BlogCard({ featured = false, post }) {
  return (
    <article className={featured ? 'grid gap-7 lg:grid-cols-[1.2fr_0.8fr] lg:items-center lg:gap-14' : ''}>
      <a className='group block overflow-hidden bg-neutral-100 outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-4' href={`/blog/${post.slug}`}>
        <img alt={post.coverAlt} className={`w-full object-cover transition-transform duration-700 group-hover:scale-[1.025] ${featured ? 'aspect-[16/10]' : 'aspect-[4/3]'}`} decoding='async' height='900' loading={featured ? 'eager' : 'lazy'} src={post.coverImage} width='1400' />
      </a>
      <div className={featured ? '' : 'border-t border-black/15 pt-5'}>
        ...
        <h2 className={`${featured ? 'mt-4 text-4xl sm:text-5xl' : 'mt-3 text-2xl'} font-semibold tracking-[-0.045em] text-black`}>
