    ...
    <section aria-label={title} className='grid gap-4 sm:grid-cols-2'>
      {images.map((image, index) => (
        <motion.figure
          ...
          initial={reducedMotion ? false : { clipPath: 'inset(0 0 18% 0)', opacity: 0, y: 22 }}
          key={`${image}-${index}`}
          transition={{ delay: reducedMotion ? 0 : Math.min(index * 0.08, 0.2), duration: reducedMotion ? 0 : motionDuration.slow, ease: motionEase }}
