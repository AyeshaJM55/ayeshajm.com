...
function PageHero({ actions, description, eyebrow, image, imageAlt, title }) {
  return (
    <section className='bg-hero-surface px-4 pb-16 pt-32 sm:px-6 sm:pb-20 sm:pt-36 lg:px-[clamp(2.5rem,4vw,4.75rem)] lg:pb-24 lg:pt-40'>
      <div className={`mx-auto grid w-full max-w-[1600px] gap-12 ${image ? 'lg:grid-cols-[minmax(0,0.9fr)_minmax(420px,0.75fr)] lg:items-end' : ''}`}>
        <div>
          ...
          <h1 className='mt-5 max-w-5xl text-[clamp(3.25rem,8vw,8.5rem)] font-semibold leading-[0.9] tracking-[-0.065em] text-black'>{title}</h1>
        ...
        {image ? (
          <div className='aspect-[4/3] overflow-hidden bg-neutral-200'>
            <img alt={imageAlt} height='900' className='size-full object-cover' width='1200' src={image} />
          </div>
        ) : null}
      </div>
    </section>
