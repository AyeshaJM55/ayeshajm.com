...
function ContactCta({ description, href = '/contact', label, title }) {
  const reducedMotion = useReducedMotion()
  const { localizePath, t } = useLocale()
  const resolvedLabel = label ?? t('common.actions.startProject')

  return (
    <section className='overflow-hidden bg-white px-4 py-16 sm:px-6 sm:py-20 lg:px-[clamp(2.5rem,4vw,4.75rem)] lg:py-24'>
      <motion.div className='mx-auto grid min-h-[36svh] w-full max-w-[1600px] bg-black text-white md:grid-cols-[minmax(0,1fr)_14rem]' initial={reducedMotion ? false : { opacity: 0, y: 30 }} transition={{ duration: reducedMotion ? 0 : motionDuration.slow, ease: motionEase }} viewport={motionViewport} whileInView={{ opacity: 1, y: 0 }}>
        <motion.div className='flex flex-col justify-center p-8 sm:p-12 lg:p-16' initial={reducedMotion ? false : { opacity: 0, y: 18 }} transition={{ delay: reducedMotion ? 0 : 0.14, duration: reducedMotion ? 0 : motionDuration.base, ease: motionEase }} viewport={motionViewport} whileInView={{ opacity: 1, y: 0 }}>
...
ContactCta.propTypes = { description: PropTypes.string.isRequired, href: PropTypes.string, label: PropTypes.string, title: PropTypes.string.isRequired }
