...
const getCurrentLocation = () => typeof window === 'undefined' ? '/' : `${window.location.pathname}${window.location.search}${window.location.hash}`

function App({ includeSeo = true, initialLoaderVisible = false, initialLocation, initialPrerendered = false, initialRouteReady = false }) {
  ...
  useEffect(() => {
    ...
    const finishInitialLoad = () => {
      ...
      }, Math.max(0, MINIMUM_SITE_LOADER_DURATION - elapsed))
    }
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setIsRouteReady(false)
    route.loadPage().catch(() => undefined).finally(() => {
